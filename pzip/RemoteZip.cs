...
/*
 * 
 * EDITED BY: Andrea Bentivegna (@blackgeektuto).
 *  - Fixed buffer overflow issues when downloading ZIPs > 2GB
 *  - Renamed some wrong void(s)
 *  
 */
...
namespace RemoteZip
{
	...
	public class RemoteZipFile : IEnumerable
	{
		...
		/// <returns></returns>
        bool FindCentralDirectory(string url, out Int64 Offset, out int Size, out int Entries)
		{
			...
			while(true)
			{
				...
				// scan for the central block. The position of the central block  is end-comment-22
				//<
				// 50 4B 05 06
				int pos = endSize-22;
				...
				else
				{
                    ...
                    if (Offset < 0)
                    {
                        /* Whoah signed int overflow. fixing :D */
                        Console.WriteLine("Meh, fixing this bad offset: " + Offset);
                        Offset = 2147483648 + (2147483648 - Math.Abs(Offset));
                    }
                    Console.WriteLine("Fixed offset is: " + Offset);
		...
		public Stream GetInputStream(ZipEntry entry)
		{
            ...
            int index = unchecked((int)entry.ZipFileIndex);
            ...
            if (entry.Offset < 0)
            {
               ...
               /* Whoah signed int overflow, bitches */
                Console.WriteLine("Fixing ZipEntry.Offset: " + entry.Offset);
                entry.Offset = 2147483648 + (2147483648 - Math.Abs(entry.Offset));

                Console.WriteLine("Fixed ZipEntry.Offset is: " + entry.Offset);
            }

            Int64 limit = (Int64)(entry.Offset + entry.CompressedSize + 16 + 65536 * 2);
			if(limit >= MaxFileOffset)
				limit = MaxFileOffset-1;
            req.AddRange((Int64)entry.Offset, limit);
			HttpWebResponse res = (HttpWebResponse)req.GetResponse();
